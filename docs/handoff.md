...
- `走势复盘`：生成前选择 `默认复盘` 或 `AI 复盘`，两者共用本地 K 线、指数/板块对比和排序证据；研究端按“市场总环境、排序总表、逐个锐评、关键转折点复盘、明日验证”组织，视频端按 `夯爆了 > 人上人 > 立棍单打 > 刷子 > 混子 > NPC > 拉完了` 输出三句话以内的锐评卡片且不写“明天”。结果区只展示一个来源。DeepSeek API Key 从 `DEEPSEEK_API_KEY` 或页面会话输入读取，不写入源码。
