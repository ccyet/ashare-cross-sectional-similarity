...
- `数据检查 / 下载`：历史页和横截面页共用 `data_check` 与 `_download_symbols_with_progress`，只补缺文件、覆盖不足、区间无数据或读取失败的标的。
...
2026-05-18 最新验证结果：`94 passed`，`ruff check .` 通过。

## 已知边界

- 指数成分、行业和概念列表来自 AkShare 当前接口，不是历史时点成分。
- 搜索结果只做研究辅助，不构成交易建议。
- OpenBB 链路是可选增强，默认仍走 `trend-backtest`。
